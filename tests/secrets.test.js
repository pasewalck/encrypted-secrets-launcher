...
import { decrypt, generateKey } from '../src/crypt.js';
...
describe('Secrets', () => {
  ...
  beforeEach(() => {
    ...
    vars = [
      new Var('DB_PASSWORD', () => 'default-db-pass'),
      new Var('API_KEY', () => 'default-api-key'),
    ];
  });
  ...
  describe('save', () => {
    it('persists encrypted secrets to file', () => {
      ...
      const decryptedDbPass = JSON.parse(
        decrypt(savedContent.encryptedSecrets.DB_PASSWORD, { key })
      );
